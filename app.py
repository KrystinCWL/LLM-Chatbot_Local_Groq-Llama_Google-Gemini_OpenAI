...
def ask_ai_with_context(user_question: str, search_results: list, ai_provider: str = "local", api_key: str = "", model: str = "") -> dict:
    ...
    # GEMINI MODE
        ...
        try:
            # client = genai.Client(api_key=api_key)  # google.generativeai 沒有 Client 類別！
            
            # Step 1: 先定義模型名稱
            gemini_model = model if model else 'gemini-2.5-flash'  # 支援你前端看到的 gemini-2.5-flash

            # Step 2: 正確！直接設定 API key（只設一次） 
            genai.configure(api_key=api_key)
            
            # Step 3: 建立模型物件（使用正確的模型名）
...
app.config['MYSQL_PASSWORD'] = os.getenv('MYSQL_PASSWORD', 'C!4rfv2wsx6yhn')  # ⚠️ CHANGE THIS!
